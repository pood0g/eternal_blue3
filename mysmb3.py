...
class MYSMB(smb.SMB):
	...
	def create_smb_packet(self, smbReq, mid=None, pid=None, tid=None):
		...
		req = pkt
		return b'\x00'*2 + pack(b'>H', len(req)) + req.getData()  # assume length is <65536
	...
	def create_nt_trans_packet(self, function, setup=b'', param=b'', data=b'', mid=None, maxSetupCount=None, totalParameterCount=None, totalDataCount=None, maxParameterCount=None, maxDataCount=None, pid=None, tid=None, noPad=False):
		if maxSetupCount is None:
			maxSetupCount = len(setup)
		if totalParameterCount is None:
			totalParameterCount = len(param)
	...
	def create_nt_trans_secondary_packet(self, mid, param=b'', paramDisplacement=0, data=b'', dataDisplacement=0, pid=None, tid=None, noPad=False):
